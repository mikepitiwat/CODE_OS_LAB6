...
int sortQ(){                //เรียงข้อมูลจากน้อยไปมาก
    Queue tempP;
    int j;
    for (int i = 1; i <SP ; ++i) {
        j = i+1;
...
int minBurt(int tempP[], int countP) {  //ในกรณีที่มีโปรเซสเกิดในเวลาเดียวกัน หาค่าโปรเซสที่ใช้เวลาทำงานน้อยที่สุด
    int minBurt = P[tempP[0]].BurtT;
    int minPro = tempP[0];
    for (int i = 0; i < countP; ++i)
        if(P[tempP[i]].BurtT < minBurt)
            minPro = tempP[i];
    for (int i = 0; i < countP; ++i)    //เก็บโปรเซสที่เหลือไว้ในคิวยกเว้น โปรเซสที่ใช้เวลาทำงานน้อยที่สุด
        if(tempP[i] != minPro)
            push(tempP[i]);
    return minPro;                      //โปรเซสที่ใช้เวลาทำงานน้อยที่สุด
}

int synchronous(int index) {            //ตรวจสอบโปรเซสเเกิดพร้อมกัน (ในเวลาเดียวกัน)
    ...
    if (countP > 1)                     //ถ้ามีโปรเซสเกิดพร้อมกัน
        return minBurt(tempP, countP);  //หาโปรเซสที่ใช้เวลาในการทำงานน้อยที่สุด
    else                                //ถ้าไม่มีโปรเซสเกิดพร้อมกัน
        return index;                   //ส่งค่าโปรเซส
}
...
void SJF_NP(){
    ...
    for (int i = 0; i <= NT; ++i) {              //i แทนเวลา (Time)
        for (int j = 1; j <= NP; ++j) {
            if(i == P[j].ArrivalT){              //ณ เวลาที่ i มีโปรเซส 1 2 3 ...N เกิดขึ้นใหม่
                ...
                if(i >= ENDPro && SP == 0){      //ณ เวลาที่ i ถ้าไม่มีโปรเซสลกำลังทำงาน(ไม่มีการใช้ทรัพยากร) และ ไม่มีโปรเซสไหนรออยู่ในคิว
                    indexP = synchronous(indexP);//ถ้ามีโปรเซสเกิดขึ้นพร้อกัน(ในเวลาเดียวกัน)
        ...
        if (i >= ENDPro && SP != 0) {            //ณ เวลาที่ i ถ้าไม่มีโปรเซสไหนทำงาน(ทรัพยากรว่าง) แต่ ยังมีโปรเซสเหลืออยู่ในคิว
            indexP = sortQ();                    //เรียงโปรเซสในคิว โดยดูจากโปรเซสที่ใช้เวลาการทำงานน้อยที่สุด

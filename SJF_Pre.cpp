    ...
    for (int i = 0; i <= NT; ++i) {              //i แทนเวลา (Time)
        for (int j = 1; j <= NP; ++j) {
            if(i == P[j].ArrivalT){              //ณ เวลาที่ i มีโปรเซส 1 2 3 ...N เกิดขึ้นใหม่
                ...
                if(i >= ENDPro && SP == 0){      //ณ เวลาที่ i ถ้าไม่มีโปรเซสลกำลังทำงาน(ไม่มีการใช้ทรัพยากร)
                    ...
                    if(P[indexP].BurtT < (P[runningP].BurtT-i)){//ให้เช็คว่าโปรเซสที่เกิดใหม่ใช้เวลาทำงานน้อยกว่า โปรเซสที่กำลังทำงานใช่หรือไม่

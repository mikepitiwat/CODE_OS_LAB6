...
int Synchronous(int index){               // ตรวจดูว่ามีโปรเซสเเกิดในเวลาเดียวกันหรือไม่
    int count = 0;
    for (int i = 1; i <= NP; i++)
        if(P[index].ArrivalT == P[i].ArrivalT)
            count++;
        ...
        for (int j = 1; j <= NP; j++) {
            if (i == P[j].ArrivalT) {                   // ณ เวลาที่ i มี process[j] เกิดขึ้น
                ...
                if (i >= END_P && (F ==0 && R == 0 )) { // ถ้าจบQuantum time = โปรเซสสามารเข้าไปทำงานได้
                    index = Synchronous(index);           // ตรวจสอบว่า มีโปรเซสเกิดขึ้นพร้อมกัน ใช่หรือไม่ (ถ้าใช่ก่ใช่ FCFS)
                    ...
                    Synchronous(index);                 // ถ้ามีโปรเซสเกิดขึ้นพร้อมกันเก็บให้เก็บโปรเซสไว้ในคิว
                    break;
                }
            }
        }
